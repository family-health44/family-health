...
export async function fetchVisits(): Promise<DbVisit[]> {
  try {
    const { data, error } = await db.from('visits').select('id, title, visit_date, doctor_id, person_id, family_group_id, pre_notes, post_notes').order('visit_date', { ascending: false });
    if (error) throw error;
    return (data ?? []).map((v: Record<string, unknown>) => ({ ...v, visit_time: null }) as import('@/shared/types/database').DbVisit);
  } catch (error) { handleNetworkError(error); }
}

export async function fetchVisitsByDateRange(startDate: string, endDate: string): Promise<DbVisit[]> {
  try {
    const { data, error } = await db.from('visits').select('id, title, visit_date, doctor_id, person_id, family_group_id, pre_notes, post_notes').gte('visit_date', startDate).lte('visit_date', endDate).order('visit_date');
    if (error) throw error;
    return (data ?? []).map((v: Record<string, unknown>) => ({ ...v, visit_time: null }) as import('@/shared/types/database').DbVisit);
  } catch (error) { handleNetworkError(error); }
}
...
export async function fetchVisitById(visitId: string): Promise<DbVisit | null> {
  try {
    const { data, error } = await db.from('visits').select('id, title, visit_date, doctor_id, person_id, family_group_id, pre_notes, post_notes').eq('id', visitId).maybeSingle();
    if (error) throw error;
...
export async function updateVisit(params: UpdateVisitParams): Promise<DbVisit> {
  try {
    const { data, error } = await db.from('visits').update({ title: params.title, visit_date: params.visitDate, doctor_id: params.doctorId, pre_notes: params.preNotes, post_notes: params.postNotes }).eq('id', params.visitId).select('id, title, visit_date, doctor_id, person_id, family_group_id, pre_notes, post_notes').single();
    if (error) throw error;
