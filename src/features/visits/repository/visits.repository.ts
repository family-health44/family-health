...
const COLS = 'id, title, visit_date, visit_time, doctor_id, person_id, family_group_id, pre_notes, post_notes, total_cost, out_of_pocket, reminder_offset_minutes, reminder_at';
...
export interface UpdateVisitParams { visitId: string; title: string; visitDate: string; visitTime: string | null; doctorId: string | null; preNotes: string | null; postNotes: string | null; totalCost: number | null; outOfPocket: number | null; reminderOffsetMinutes: number | null; reminderAt: string | null; }

export async function updateVisit(params: UpdateVisitParams): Promise<DbVisit> {
  try {
    const { data, error } = await db.from('visits').update({ title: params.title, visit_date: params.visitDate, visit_time: params.visitTime, doctor_id: params.doctorId, pre_notes: params.preNotes, post_notes: params.postNotes, total_cost: params.totalCost, out_of_pocket: params.outOfPocket, reminder_offset_minutes: params.reminderOffsetMinutes, reminder_at: params.reminderAt }).eq('id', params.visitId).select(COLS).single();
    if (error) throw error;
