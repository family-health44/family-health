// src/features/visits/repository/visits.repository.ts
...
export async function fetchVisits(): Promise<DbVisit[]> {
  try {
    const { data, error } = await supabase
      .from('visits')
      .select('id, title, visit_date, doctor_id, person_id, family_group_id, pre_notes, post_notes')
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        pre_notes: params.preNotes,
        post_notes: params.postNotes,
      })
      .select('id, title, visit_date, doctor_id, person_id, family_group_id, pre_notes, post_notes')
      .single();
    if (error) throw error;
