...
export const PersonDetailScreen = () => {
  ...
  const { addVisit, isAdding: isAddingVisit } = useVisits();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F7F5F0' }}>
      <View style={{ paddingTop: insets.top + 4, paddingHorizontal: 16, paddingBottom: 6, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#F7F5F0' }}>
        ...
        <PressableBase accessibilityRole="button" accessibilityLabel="Edit person" style={(pressed) => ({ width: 32, height: 32, borderRadius: 16, backgroundColor: '#EEEAE3', alignItems: 'center', justifyContent: 'center', opacity: pressed ? 0.6 : 1 })}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 14 }}>
        ...
        <View style={{ backgroundColor: 'white', borderWidth: 1, borderColor: '#E3DDD5', borderRadius: 14, overflow: 'hidden' }}>
          {menuItems.map((item, index) => (
            <PressableBase key={item.key} onPress={() => item.route ? router.push(item.route as never) : null} accessibilityRole="button" accessibilityLabel={item.label} style={(pressed) => ({ flexDirection: 'row', alignItems: 'center', padding: 14, borderBottomWidth: index < menuItems.length - 1 ? 1 : 0, borderBottomColor: '#F0EDE8', backgroundColor: pressed ? '#F7F5F0' : 'white', gap: 12 })}>
      ...
      <AddTodoModal visible={showTodoModal} isLoading={isAddingTodo} defaultPersonId={person.id} onAdd={async (input) => { await addTodo(input); setShowTodoModal(false); }} onDismiss={() => setShowTodoModal(false)} />
      <AddVisitModal visible={showVisitModal} isLoading={isAddingVisit} defaultPersonId={person.id} onAdd={async (input) => { await addVisit(input); setShowVisitModal(false); }} onDismiss={() => setShowVisitModal(false)} />
    </View>
  );
};
